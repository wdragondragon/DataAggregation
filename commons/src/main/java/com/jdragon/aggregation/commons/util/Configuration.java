...
public class Configuration {
    ...
    public static Configuration from(String json) {
        ...
        try {
            ...
            throw AggregationException.asException(CommonErrorCode.CONFIG_ERROR,
                    e);
        }
    ...
    public static Configuration from(File file) {
        try {
            return Configuration.from(IOUtils
                    .toString(new FileInputStream(file), Charset.defaultCharset()));
    ...
    public static Configuration from(InputStream is) {
        try {
            return Configuration.from(IOUtils.toString(is, Charset.defaultCharset()));
        } catch (IOException e) {
            throw AggregationException.asException(CommonErrorCode.CONFIG_ERROR,
    ...
    public String getNecessaryValue(String key, ErrorCode errorCode) {
        ...
        if (StringUtils.isBlank(value)) {
            throw AggregationException.asException(errorCode,
    ...
    public Object remove(final String path) {
        ...
        if (null == result) {
            throw AggregationException.asException(
                    ...
                    String.format("配置文件对应Key[%s]并不存在，该情况是代码编程错误. 请联系引擎团队的同学.", path));
        }
    ...
    private void setObject(final String path, final Object object) {
        ...
        throw AggregationException.asException(CommonErrorCode.RUNTIME_ERROR,
                String.format("值[%s]无法适配您提供[%s]， 该异常代表系统编程错误, 请联系引擎开发团队!",
    ...
    Object buildObject(final List<String> paths, final Object object) {
        if (null == paths) {
            throw AggregationException.asException(
                    CommonErrorCode.RUNTIME_ERROR,
                    "Path不能为null，该异常代表系统编程错误, 请联系引擎开发团队 !");
        }
        ...
        for (int i = paths.size() - 1; i >= 0; i--) {
            ...
            throw AggregationException.asException(
                    CommonErrorCode.RUNTIME_ERROR, String.format(
                            "路径[%s]出现非法值类型[%s]，该异常代表系统编程错误, 请联系引擎开发团队! .",
    ...
    private Object findObjectInMap(final Object target, final String index) {
        ...
        if (null == result) {
            throw new IllegalArgumentException(String.format(
                    "您提供的配置文件有误. 路径[%s]值为null，引擎无法识别该配置. 请检查您的配置并作出修改.", index));
        }
    ...
    private Object findObjectInList(final Object target, final String each) {
        ...
        if (!StringUtils.isNumeric(index)) {
            throw new IllegalArgumentException(
                    String.format(
                            "系统编程错误，列表下标必须为数字类型，但该节点发现实际类型是[%s] ，该异常代表系统编程错误, 请联系引擎开发团队 !",
                            index));
        }
    ...
    private Configuration(final String json) {
        try {
            ...
            throw AggregationException.asException(CommonErrorCode.CONFIG_ERROR,
                    String.format("配置信息错误. 您提供的配置信息不是合法的JSON格式: %s . 请按照标准json格式提供配置信息. 错误配置如下：%s ", e.getMessage(), json));
        }
    }
