 ...
 * @since 2026/3/27
 */
public class SHA256Str extends Transformer {
   int columnIndex;

   public SHA256Str() {
      super.setTransformerName("SHA256_str");
   }
   ...
   public Record evaluate(Record record, Object... paras) {
      ...
      try {
         if (paras.length != 2) {
            throw new RuntimeException("SHA256_str paras must be 2");
         }
      ...
      try {
         ...
         if (StringUtils.isBlank(oriValue)) {
            ...
            if (StringUtils.isBlank(key)) {
               oriValue = getSHA256StrJava(oriValue);
            } else {
               oriValue = getSHA256StrJava(oriValue + key);
            }
